...
def create_labels(num_tasks, init_increment, num_classes_per_task):
    tasks_order = np.arange(init_increment + (num_tasks-1)*num_classes_per_task)
    labels = []
    if init_increment == num_classes_per_task:
        labels = tasks_order.reshape((num_tasks, num_classes_per_task))
    else:
        labels.append([tasks_order[:init_increment]])
        labels += list( tasks_order[init_increment:].reshape((num_tasks-1, num_classes_per_task)) )
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, args, device, stride=1, option='A'):
        ...
        if stride != 1 or in_planes != planes:
            if option == 'A':
                ...
                self.shortcut = nn.Sequential(
                     ...
                     MultitaskBN(self.expansion * planes)
                )
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, args, tasks_order, device, task_id=0):
        ...
        self.init_increment = args.init_increment
        self.args = args
        self.num_tasks = 0
        self.device = device

        self.task_id = task_id
        self.task_labels = create_labels((args.num_classes//args.num_classes_per_task)//2+1, args.init_increment, args.num_classes_per_task)

        self.conv1 = nn.Conv2d(3, 16, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv1_masks = torch.ones(16, 3, 3, 3)
        self.bn1 = MultitaskBN(16, args.num_tasks)
        ...
        self.linear_masks = [torch.zeros(self.num_classes, 64), torch.zeros(self.num_classes)]
    ...
    def _add_mask(self, task_id):
        ...
        self.tasks_masks[-1][-1][-1][self.task_labels[task_id]] = 1
        self.tasks_masks[-1][-1][-2][self.task_labels[task_id], :] = 1
    ...
    def features(self, x, last_relu=False):
        active_conv = self.conv1.weight*self.tasks_masks[self.task_id][0].to(self.device)
...
def resnet32(args, tasks_order, device):
    return ResNet(BasicBlock, [5, 5, 5], args, tasks_order, device)
...
def init_model(args, tasks_order, device):
    if args.model_name == 'resnet20':
        model = resnet20(args, tasks_order, device);
    elif args.model_name == 'resnet32':
        model = resnet32(args, tasks_order, device);
    elif args.model_name == 'resnet44':
        model = resnet44(args, tasks_order, device);
    elif args.model_name == 'resnet56':
        model = resnet56(num_classes, num_classes_per_task, device);
    elif args.model_name == 'resnet110':
        model = resnet110(num_classes, num_classes_per_task, device);
...
def validate(valid_loader, model, criterion, task_id, device):
    ...
    increment = model.num_classes_per_task if task_id > 0 else model.init_increment
    increment = model.num_classes_per_task if task_id > 0 else model.init_increment
    if task_id > 0:
        offset_a = model.init_increment + (task_id-1)*model.num_classes_per_task
    else:
        offset_a = 0
    
    offset_b = offset_a + increment
...
def train(args, model, train_loader, test_loader, device, task_id=0):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                     ...
                                                     gamma=args.gamma)
    #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.train_epochs)
...
def resnet_conv_block_pruning(net, layer_num, block_num, conv_num, alpha, x_batch, task_id, residual=0):
    ...
    bn_out = bn(F.conv2d(x_batch, weight=active_conv, bias=None, stride=conv.stride, padding=conv.padding), task_id)

    if conv_num == 1:
        block_out = bn_out + residual     # F.relu(bn_out + residual)
    else:
    ...
    #print('Accuracy {}'.format(name), np.round(100 * accuracy(net, test_loader, device), 2))
...
def resnet_backward_pruning(net, task_id):
                ...
                for conv_num in range(2)[::-1]:
                    if (('layer{}.{}.bn{}.bns.{}'.format(layer+1, block, conv_num+1, task_id) in name) and ('num_batches_tracked' not in name)):
                        ...
                        if (conv_num == 1):
                            ...
                            pruned_channels = torch.nonzero((1 - net.tasks_masks[task_id][1][layer][block][-1][start:end])*(
                                        net.tasks_masks[task_id][1][layer][block][conv_num].sum(dim=(0, 2, 3)) == 0)).reshape(1, -1).squeeze(0)
    ...
    net.state_dict()['bn1.bns.{}.running_mean'.format(task_id)][pruned_channels] = 0
    net.state_dict()['bn1.bns.{}.running_var'.format(task_id)][pruned_channels] = 0 
...
def resnet_pruning(net, alpha_conv, x_batch, task_id, device,
    ...
    #print('---Before backward: ', np.round(100 * accuracy(net, test_loader, device), 2))
    net = resnet_backward_pruning(net, task_id)
    #print('---After backward: ', np.round(100 * accuracy(net, test_loader, device), 2))  
...
def iterative_pruning(args, net, train_loader, test_loader, x_prune, task_id, device, path_to_save,
    ...
    for it in range(1, args.num_iters + 1):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                         ...
                                                         gamma=args.gamma)
        #scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.retrain_epochs)
        loss = torch.nn.CrossEntropyLoss()
...
def get_prototypes(model):
    ...
    for task_id in range(model.num_tasks):
        ...
        x = torch.FloatTensor(train_dataset[task_id].data)[idx]
        x = x.permute(0, 3, 1, 2)
        x = torchvision.transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225))(x.float()/255)
...
def select_subnetwork_icarl(model, x, prototypes, num_learned=10):
    ...
    for task_id in range(num_learned):
        set_task(model, task_id)
        out = features_out(model, x, last_relu=False)
...
def select_subnetwork_maxoutput(model, x, num_learned, device):
    ...
    for task_id in range(num_learned):
        ...
        if task_id > 0:            
            max_out.append(torch.max(preds[:, model.init_increment+(task_id-1)*model.num_classes_per_task: model.init_increment+task_id*model.num_classes_per_task], dim=1)[0].sum().cpu().detach())
        else:
            max_out.append(torch.max(preds[:, :model.init_increment], dim=1)[0].sum().cpu().detach())    
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default='resnet32', help='network architecture to use')
    ...
    parser.add_argument('--init_increment', type=int, default=50, help='number of classes in the first task')
    ...
    parser.add_argument('--test_batch_size', type=int, default=60, help='number of examples per test batch')
    parser.add_argument('--train_epochs', type=int, default=160, help='number training epochs')      # 120-sgd, 70-adam
    parser.add_argument('--retrain_epochs', type=int, default=50, help='number of retraining epochs after pruning') #50
    parser.add_argument('--optimizer', type=str, default='adam', help='optimizer')
    parser.add_argument('--lr_decay_type', type=str, default='multistep', help='learning rate decay type')
    parser.add_argument('--lr', type=float, default=1e-3, help='initial learning rate')
    parser.add_argument('--decay_epochs_train', nargs='+', type=int, default=[80, 120], help='epochs for multistep decay')  # 60, 90-sgd; 20, 40, 60-adam
    parser.add_argument('--decay_epochs_retrain', nargs='+', type=int, default=[20, 35], help='epochs for multistep decay')  # 20, 40 - sgd; 15,25,40 - adam
    parser.add_argument('--gamma', type=float, default=0.1, help='multiplicative factor of learning rate decay') # 0.1-sgd; 0.2-adam
    ...
    parser.add_argument('--task_select_method', type=str, default='IS', help='Task selection method')
    ...
    if TRAIN:
        task_labels = create_labels(args.num_tasks, args.init_increment, args.num_classes_per_task)
        train_dataset, test_dataset = task_construction(task_labels, args.dataset_name, tasks_order)

        net = init_model(args, tasks_order, device)
